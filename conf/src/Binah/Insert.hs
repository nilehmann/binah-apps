...
assume insert :: forall < p :: Entity record -> Bool
                        ...
                        >.
  { rec :: (Entity<p> record) |- {v: (Entity User) | v == currentUser} 
      <: {v: (Entity<insertpolicy rec> User) | True}}

  { rec :: (Entity<p> record) |- {v: (Entity<querypolicy p> User) | True} 
      <: {v: (Entity<audience> User) | True}}
